...
insert into tag2post (post_id, tag_id) values
 ((SELECT id FROM posts WHERE title like 'MyFirstPost'), (SELECT id FROM tags WHERE name like 'lookAtMe')),
 ((SELECT id FROM posts WHERE title like 'MySecondPost'), (SELECT id FROM tags WHERE name like 'Java'));
