...
insert into posts (is_active, moderation_status, moderator_id, user_id, publication_time, title, text, view_count)
    values ( 1, 'ACCEPTED', (SELECT id FROM users WHERE is_moderator like 1),
            (SELECT id FROM users WHERE is_moderator like 0), NOW(), 'MyFirstPost', 'Test post text', 1),
            ( 0, 'ACCEPTED', (SELECT id FROM users WHERE is_moderator like 1),
                (SELECT id FROM users WHERE name like 'ivan'), NOW(), 'MySecondPost', 'Test post text', 1),
            ( 1, 'ACCEPTED', (SELECT id FROM users WHERE is_moderator like 1),
                (SELECT id FROM users WHERE name like 'eugene'), NOW(), 'SimplePost', 'Simple post text', 2),
            ( 1, 'ACCEPTED', (SELECT id FROM users WHERE is_moderator like 1),
                (SELECT id FROM users WHERE name like 'eugene'), NOW(), 'ThirdPost', 'Third post text', 3);
